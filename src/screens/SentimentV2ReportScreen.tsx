...
import {
  ...
  BRAND_SENTIMENT_TREND,
  type SentimentSwotPlatform,
  type SentimentPlatform,
...
export function SentimentV2ReportScreen() {
  ...
  const [brandSentimentPlatform, setBrandSentimentPlatform] = useState<SentimentPlatform>('ChatGPT')
  const [brandSentimentDateRange, setBrandSentimentDateRange] = useState('Last 3 months')
        ...
        <div className="flex flex-col gap-xl px-2xl py-xl">
          ...
          {(() => {
            const DATE_RANGE_MONTHS: Record<string, number> = {
              'Last 3 months': 3, 'Last 6 months': 6, 'Last 12 months': 12, 'Last 24 months': 24,
            }
            const brandSentimentSeries = [
              { key: 'aspendentalSentiment', label: 'Aspendental',  color: chartColors.categorical[0] },
              { key: 'fleurChoiceSentiment', label: 'Fleur Choice', color: chartColors.categorical[1] },
              { key: 'wellYesNowSentiment',  label: 'WellYesNow',   color: chartColors.categorical[2] },
            ]
            const brandTrendData    = BRAND_SENTIMENT_TREND[brandSentimentPlatform]
            const visibleBrandTrend = brandTrendData.slice(-Math.min(DATE_RANGE_MONTHS[brandSentimentDateRange], brandTrendData.length))
            const platformTabs = [
            ...
            return (
              <ChartCard
                title="What is the sentiment score of your brands over time?"
                subtitle="Track positive sentiment across your brands in answers generated by AI sites over time."
                toolbar={
                  <div className="flex items-center gap-sm">
                    <DateRangeSelector
                      value={brandSentimentDateRange}
                      options={['Last 3 months', 'Last 6 months', 'Last 12 months', 'Last 24 months']}
                      onChange={setBrandSentimentDateRange}
                    />
              ...
              >
                <div className="mb-lg">
                  <CardTabs
                    tabs={platformTabs}
                    activeTab={brandSentimentPlatform}
                    onChange={(id) => setBrandSentimentPlatform(id as SentimentPlatform)}
                  />
                </div>
                <TrendLineChart
                  data={visibleBrandTrend}
                  series={brandSentimentSeries}
                  height={280}
                  yTickFormatter={(v) => `${v}%`}
                />
                <div className="flex flex-wrap items-center gap-xl mt-sm px-xs">
                  {brandSentimentSeries.map((s) => (
                    <div key={s.key} className="flex items-center gap-xs">
                      <span className="inline-block size-3 rounded-full" style={{ backgroundColor: s.color }} />
          ...
          {/* 5. Sentiment across sites — hidden for now, kept for future use */}
          {false && (
