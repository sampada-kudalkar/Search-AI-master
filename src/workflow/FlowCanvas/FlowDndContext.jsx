...
const FlowDndStateContext = createContext({ isDraggingFromLHS: false });

export function useFlowDndState() {
  return useContext(FlowDndStateContext);
}
...
function LhsDragPreview({ preview }) {
  if (!preview) return null;
  const { label, icon, svgIcon } = preview;

  return (
    <div className="lhs-drag-preview">
      {svgIcon ? (
        <span className="lhs-drag-preview__icon" style={{ display: 'flex', alignItems: 'center', color: '#212121' }}>
          <ProcedureSvgIcon />
        </span>
      ) : icon ? (
        <span className="lhs-drag-preview__icon material-symbols-outlined">{icon}</span>
      ) : null}
      <span className="lhs-drag-preview__label">{label}</span>
      <span className="lhs-drag-preview__handle material-symbols-outlined">drag_indicator</span>
    </div>
  );
}

export function FlowDndProvider({ children, onDropNode, viewOnly = false }) {
  ...
  const handleDragStart = useCallback((event) => {
    ...
    setActiveDragPreview(event.active?.data?.current?.dragPreview ?? null);
  }, []);

  const handleDragEnd = useCallback((event) => {
    setIsDraggingFromLHS(false);
    setActiveDragPreview(null);
    if (viewOnly || !onDropNode) return;

    const { active, over } = event;
    if (!over || !active?.data?.current?.type) return;

    const overId = String(over.id);
    if (!overId.startsWith('connector-')) return;
    ...
    onDropNode({
      ...
      afterNodeId: dropData.afterNodeId ?? null,
      branchPathId: dropData.branchPathId,
      loopBodyId: dropData.loopBodyId,
      insertAtBeginning: dropData.insertAtBeginning,
    });
  }, [onDropNode, viewOnly]);
  ...
  return (
    <FlowDndStateContext.Provider value={{ isDraggingFromLHS }}>
      <DndContext
        sensors={sensors}
      ...
      >
        ...
        <DragOverlay dropAnimation={null}>
          {activeDragPreview ? <LhsDragPreview preview={activeDragPreview} /> : null}
        </DragOverlay>
      </DndContext>
    </FlowDndStateContext.Provider>
  );
}
