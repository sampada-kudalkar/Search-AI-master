...
function ScatterLegend({ competitorSeries }: { competitorSeries: { name: string }[] }) {
  const allEntries = [{ name: 'You' }, ...competitorSeries]
  return (
    <div className="mt-lg flex flex-wrap gap-lg">
      {allEntries.map((entry, i) => (
        <span key={entry.name} className="inline-flex items-center gap-xs text-small text-text-secondary">
          <span
      ...
      <ResponsiveContainer width="100%" height={340}>
        <ScatterChart margin={{ top: 8, right: 24, bottom: 40, left: 70 }}>
          ...
          <Scatter
            ...
            opacity={0.85}
            r={5}
            onClick={(data) => onDotClick?.(data as ByLocationDot)}
