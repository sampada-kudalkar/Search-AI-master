...
| Toggle | components/Toggle/Toggle.tsx | checked, onChange, disabled? — small pill switch (16×32px), `role="switch"`; shared version of the toggle previously duplicated inline in AgentSettingsTab/ContextModal |
| ThemesPromptsTable | components/ThemesPromptsTable/ThemesPromptsTable.tsx | themes: ThemeConfig[], onEditPrompt?, onLocationsClick?, onAddPrompt? — expandable theme/prompt DataTable (chevron-toggle theme rows, indented prompt rows) with Locations/Monthly searches/AI sites/Tracking status/Updated on columns, tracking-status pills (Tracking/Not tracking/Starts next cycle via new Chip `info` variant), row-hover Edit + 3-dot (Stop tracking, Delete) |
...
| BrandDrawer | components/BrandDrawer/BrandDrawer.tsx | open, mode: 'add' \| 'edit', initialValues?: BrandDrawerValues (domainUrls: string[]), heading?: string, hideBrandKit?, hideVariations?, hideDomainUrl?, multiDomain?, onClose, onSave(values) — 650px right drawer for the Brands settings page: back-arrow + title (defaults to "Add"/"Edit", or `heading` override — pass the brand name when opened via row click, "Edit {name}" when opened via the edit action) + Cancel/Save header (Save disabled until dirty), Name input, Domain URL field (single input when `multiDomain` is unset — used for `OtherBrand`; or, when `multiDomain` is set — used for the primary `Brand` entity — a repeatable list of label+input+remove-button rows with a "+ Add domain" add-row, backed by `domainUrls: string[]`), Brand variations chip-input (type + Enter to add, click x to remove), Brand kit section listing per-brand `BrandKitInstance` rows (name + location scope, hover-reveal edit/remove icon buttons) with an inline "+ Add brand kit" add-row (no modal) — sourced entirely from `Brand.brandKits` in `src/data/brandsData.ts`, no external catalog prop |
| DomainListCell | components/DomainListCell/DomainListCell.tsx | domains: string[], maxVisible?=7 — renders a brand's domains as neutral `Chip`s (same style as the Variations column, non-interactive — no link-out); beyond `maxVisible` domains shows a "+N more" text toggle that expands the full chip list inline ("Show less" to collapse); used in `BrandV1Screen`'s "Website" column only — multi-domain support (and this cell) is scoped to Brand V1; the non-V1 `BrandsScreen` stays single-domain plain text |
| Slider | components/Slider/Slider.tsx | value, onChange, min?=0, max?=100, defaultValue?, color?, className? — draggable pointer-events track slider with a "Default" label shown under the handle when value equals defaultValue; used for score-weighting rows (e.g. Search AI score settings) |
| DomainHealthScoreHeader | components/DomainHealthScoreHeader/DomainHealthScoreHeader.tsx | healthAvg: number \| null, breakdown: { ai, disc, fresh } — 4-col "Health" big-number card + "Health consists of" 3-metric breakdown card (AI readiness/Discoverability/Freshness), each with an info-tooltip icon; shared between the Domain health report's domain-audit and page-details drill-downs |
| DomainHealthScoreHeaderV2 | components/DomainHealthScoreHeader/DomainHealthScoreHeaderV2.tsx | healthAvg: number \| null, breakdown: { ai, disc, fresh, issues } — same 4-col shell as DomainHealthScoreHeader but with a 4th "Issues & warnings" stat and the shared `InfoTooltip` component (instead of the inline hover tooltip); used by the Domain health v2 domain and page-detail screens |
| HorizontalBarChart | components/charts/HorizontalBarChart.tsx | data: HorizontalBarDatum[] ({label,value,color}), rowHeight?=36, maxHeight?=280, max?=100 — single-series Recharts bar chart laid out horizontally (`layout="vertical"`, category y-axis, no x-axis), wrapped in a fixed-`maxHeight` scrollable container so any number of rows stay reachable via internal scroll; used by Domain health v2's page-detail "Score breakdown" card |
| DetailCard | components/DetailCard/DetailCard.tsx | title, children, className? — generic 400px-tall bordered card shell (title + scrollable body); used for the Domain health report's Crawlability/Blocked folders/Site-wide technical health/Issues & warnings/AI bot access/Content/Technical cards (rows composed by the caller) |
| HealthScoreTrendCard | components/HealthScoreTrendCard/HealthScoreTrendCard.tsx | data: TrendPoint[], series: SeriesConfig[] — CardHeader (title+subtitle) + DateRangeSelector + AI-summary icon + MoreMenu toolbar, TrendLineChart, hand-rolled legend row; styled to match CitationShareCard. Used for Domain health's "Health score over time" |
| SummarizeIcon | components/SummarizeIcon/SummarizeIcon.tsx | size?=16, className? — purple sparkle glyph used for every card header's "Summarize" toolbar button (next to the 3-dot menu) and the SentimentSwotGrid center badge. Distinct from `AiIcon` (the circular purple "AI" badge), which is used elsewhere (e.g. ReportSettingsScreen's AI-site logo fallback) |
| SentimentSwotGrid | components/SentimentSwotGrid/SentimentSwotGrid.tsx | strengths, weaknesses, opportunities, threats: `{title,description}[]` — single grey box (`bg-surface-l2`) 2x2 layout with a cross-divider and a centered circular badge (bordered `bg-surface` circle + `SummarizeIcon`) at the intersection; each quadrant has a corner-positioned `Chip` pill (Strengths green top-left, Weaknesses danger top-right, Opportunities/Threats neutral bottom-left/right) and title/description item rows (color-only hierarchy, no bold per §6.6); used per-AI-model tab on the Sentiment report's Overview tab |
| SentimentComparisonMatrix | components/SentimentComparisonMatrix/SentimentComparisonMatrix.tsx | traits: string[], competitors: {name,isYou?}[], values: Record<trait, Record<competitorName, boolean>> — traits × competitors grid built on `DataTable` (competitors as columns, check icon for present); used for the Sentiment report's "Competitive strengths and weakness matrix" |
| CitationSentimentDrawer | components/CitationSentimentDrawer/CitationSentimentDrawer.tsx | open, onClose, row: SentimentCitationRow \| null — read-only 650px right drawer (shell copied from `IntakeFormPreviewDrawer`: overlay + aside + back-arrow header) showing a citation's category/positive sentiment/claim occurrence + strengths/weaknesses `Chip` lists |
| CompetitorSentimentDrawer | components/CompetitorSentimentDrawer/CompetitorSentimentDrawer.tsx | open, onClose, row: SentimentCompetitorRow \| null — same `IntakeFormPreviewDrawer` shell, showing a competitor's sentiment score + strengths/weaknesses `Chip` lists |
| MostMentionedTraitsCard | components/MostMentionedTraitsCard/MostMentionedTraitsCard.tsx | rows: SentimentTraitRow[] — "Most mentioned traits across all locations" card (Sentiment report Brand tab, last card); inline decorative location dropdown in title (same pattern as the Overview tab's SWOT card); expandable trait rows (chevron-toggle, same flatten/expand convention as `SentimentPromptTab`) revealing claim/statement sub-rows; Occurrences column shows count · % with an `InfoTooltip`; Cited websites column shows overlapping initials-circle avatars (`getInitials`/`getCompetitorColor`) with a native hover tooltip; Sentiment column shows a numeric % on trait rows and a Positive/Negative `Chip` pill on claim sub-rows |
